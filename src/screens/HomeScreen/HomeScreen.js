...
const HomeScreen = (props) => {
  ...
  useEffect(async () => {
    if (props.products == null) {
      ...
      let Number_of_pages = Math.floor(
  ...
  const ClearFilter = () => {
    ...
    props.search_image_product_action(null);
  };
  ...
  const handleFileSelect = (event) => {
    ...
    searchimageHandler(event.target.files[0]);
  };

  const searchimageHandler = async (file) => {
    // event.preventDefault()
    setLoading(true);
    response = await SearchImageHandler(file);
    if (response.status === 200) {
      setLoading(false);
      const searchimageresult = response.data.products;
      props.search_image_product_action(searchimageresult);
  ...
  return (
    ...
    >
      ...
      return (
        ...
        <Searchbar
          imageSelector={handleFileSelect}
          filter={SearchTextProductHandler}
        />
      </Box>
      <Carusal />
      <Button
        onClick={ClearFilter}
        style={{ marginLeft: "3.6em" }}
      ...
      <Box className={classes.StyleProduct}>
        <Grid container spacing={6}>
          {props.filterproduct
            ? props.filterproduct.map((product) => {
                return (
            ...
            : props.image_Result
            ? props.image_Result.map((product) => {
                return (
                  <Grid item key={product.id} sm={12} md={6} lg={4} xl={3}>
                    <CARD product={product} />
                  </Grid>
...
const mapStateToProps = (state) => {
  return {
    ...
    image_Result: state.search_image_product_reducer.image_Result,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    search_image_product_action: (image_Result) =>
      dispatch(productAction.search_image_product_action(image_Result)),
  };
};
